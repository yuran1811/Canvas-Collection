...
class SnowBall {
  ...
  inView() {
    const inViewWidth = this.x + this.radius <= innerWidth && this.x - this.radius >= 0;
    const inViewHeight = this.y + this.radius <= innerHeight && this.y - this.radius >= 0;
  ...
  increaseAcceleration(key, value) {
    ...
    if (this.acceleration[key] + value <= MAX_ACCELERATION || this.acceleration[key] + value >= -MAX_ACCELERATION)
      this.acceleration[key] += value;
    else {
      if (this.acceleration[key] + value > MAX_ACCELERATION) this.acceleration[key] = MAX_ACCELERATION;
      else if (this.acceleration[key] + value < -MAX_ACCELERATION) this.acceleration[key] = -MAX_ACCELERATION;
    }
  }
...
const init = () => {
  ...
  for (let i = 0; i < numSnowBall; i++) snowBalls.push(new SnowBall(getNewSnowBall()));
};
